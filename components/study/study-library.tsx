...
import type { StudyBook } from "@/lib/study";
...
export default function StudyLibrary() {
  ...
  async function loadBooks() {
    const response = await fetch("/api/study/books", { cache: "no-store" });
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#fffdf5_45%,#ffffff_100%)] px-4 py-8 md:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
              Загружайте учебник, открывайте нужную страницу и работайте почти как внутри
              оригинального пособия: структура страницы рендерится приложением, а ассистент
              помогает только по вашему клику.
            </p>
          </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[440px_minmax(0,1fr)]">
          ...
          >
            <h2 className="text-2xl font-semibold text-slate-950">Загрузить учебник</h2>
            <p className="mt-2 text-sm text-slate-600">
              Поддерживаются PDF и отдельные изображения страниц.
            </p>
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  Разметка создаётся постранично и только по запросу, когда вы открываете страницу.
                </p>
              </div>
              <div className="rounded-full bg-slate-100 px-4 py-2 text-sm text-slate-600">
                {books.length} {books.length === 1 ? "учебник" : books.length < 5 ? "учебника" : "учебников"}
              </div>
            </div>
